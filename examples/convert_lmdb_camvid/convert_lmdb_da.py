...
f = open('camvid_train_da.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('camvid_train-lmdb-da', map_size=int(1e10))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
        ...
        im = Image.open('../../data/camvid/701_StillsRaw_full/'+in_) # load image
...
f = open('camvid_gt_train_da.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('camvid_train-gt-lmdb-da', map_size=int(1e10))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
        im = Image.open('../../data/camvid/LabeledApproved_full/'+in_) # load image
...
in_db = lmdb.open('camvid_test-lmdb', map_size=int(1e10))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
        # load image:
        # - as np.uint8 {0, ..., 255}
...
in_db = lmdb.open('camvid_test-gt-lmdb', map_size=int(1e10))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
        im = Image.open(in_) # load image
        im = im.resize((int(im.size[0]*0.5),int(im.size[1]*0.5)),Image.NEAREST) # downsize for reduced memory usage
