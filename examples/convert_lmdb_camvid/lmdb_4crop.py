...
f = open('camvid_train_da2.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('camvid_train-lmdb-da2', map_size=int(1e10))
...
f = open('camvid_gt_train_da2.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('camvid_train-gt-lmdb-da2', map_size=int(1e10))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
