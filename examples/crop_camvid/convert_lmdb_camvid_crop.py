...
f = open('camvid_train_c.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('camvid_train-crop-lmdb', map_size=int(1e9))
...
f = open('camvid_gt_train_c.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('camvid_train-gt-crop-lmdb', map_size=int(1e9))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
        im = Image.open(in_) # load image
