...
base_weights = '../../alexnet32/alexnetfc.caffemodel'
#test1 = bias = 0 and learning rate = 1e6 multiple stages
#test2 = bias = 1 and learning rate = 1e8 multiple stages
#test3 = bias = 1 and learning rate = 1e4 single stage
#test4 = bias = 1 and learning rate = 1e4 single stage
# init
caffe.set_mode_gpu()
...
for it in range(niter):
    ...
    if it % 1000 ==0:
        ...
        solver.net.save('fcn-alexnet-8s/fcn-alexnet-bvlc_camvid_finetune_iter_' + repr(it) + '.caffemodel')
        np.save('fcn-alexnet-8s/loss-alexnet-bvlc_camvid_finetune_iter_' + repr(it) + '.npy',train_loss)
print 'done'
